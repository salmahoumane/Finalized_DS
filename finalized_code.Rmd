...
filtered_table <- filtered_table %>%
  mutate(
    Variable_Pairs = case_when(
      ...
      Var1 == "height" & Var2 == "male" ~ "Height & Male",
      Var1 == "net_income" & Var2 == "work_time_weekly" ~ "Income & Weekly Working Hours",
      Var1 == "health" & Var2 == "health_in_2yrs" ~ "Health & Health in 2 years",
      Var1 == "height" & Var2 == "weight" ~ "Height & Weight",
      Var1 == "health_satisfaction" & Var2 == "health_in_2yrs" ~ "Health Satisfaction & Health & Health in 2 years",
      Var1 == "health_satisfaction" & Var2 == "life_satisfaction" ~ "Health Satisfaction & Life Satisfaction",
      Var1 == "health_satisfaction" & Var2 == "worr_health_categorical" ~ "Health Satisfaction & Worrying about Health",
      Var1 == "health" & Var2 == "worr_health_categorical" ~ "Health & Worrying about Health",
      Var1 == "unemp_dummy" & Var2 == "work_time_weekly" ~ "Unemployment & Weekly Working Hours",
...
ggplot(filtered_table, aes(x = reorder(Variable_Pairs, Freq), y = Freq, fill = Freq > 0)) +
  ...
  theme(
    ...
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5) # Center and style the title
  )
...
ggplot(main_unique, aes(x = factor(male), y = health, fill = factor(male))) +
  geom_violin(trim = FALSE, alpha = 0.7) + # Add transparency to the violins
  stat_summary(fun = "mean", geom = "point", shape = 16, size = 3, color = "black") + # Use a plain filled circle for the mean
  geom_text(stat = "summary", fun = "mean", aes(label = round(after_stat(y), 2)), vjust = -0.5) + # Add mean values on violins
  scale_x_discrete(labels = c("0" = "Female", "1" = "Male")) + # Update x-axis labels
  labs(title = "Health Distribution by Gender", x = "Gender", y = "Health") +
  theme_minimal(base_size = 14) +
  theme(
    panel.grid = element_blank(), # Remove background grid
...
ggplot(main_unique, aes(x = factor(male), y = health_decline_2yrs, fill = factor(male))) +
  ...
  scale_fill_manual(values = c("pink", "blue"), labels = c("Female", "Male"), name = "Gender") +
...
ggplot(main_unique, aes(x = health_satisfaction, fill = factor(male), color = factor(male))) +
  geom_density(alpha = 0.5) + # Add transparency for overlapping areas
...
ggplot(main_educ %>% filter(educ != 0), aes(x = factor(educ), y = health_satisfaction, fill = factor(educ))) +
  ...
  scale_fill_brewer(palette = "Paired", guide = "none") + # Hide legend
  labs(
...
ggplot(heatmap_data, aes(x = net_income_bin, y = 1, fill = avg_health_satisfaction)) +
  ...
  scale_fill_gradient(low = "lightblue", high = "blue", name = "Avg Satisfaction") +
  labs(
    title = "Average Health Satisfaction by Income Group",
    x = "Net Income (Binned)",
    y = "",
    fill = "Avg Health Satisfaction"
  ) +
  theme_minimal(base_size = 14) +
  theme(
...
ggplot(main, aes(x = age, y = net_income)) +
  geom_point(alpha = 0.6, color = "#4BADA9") + # Semi-transparent points
  geom_smooth(method = "loess", color = "red", se = TRUE) + # Add a LOESS trend line
  labs(
...
ggplot(main_rel, aes(x = factor(rel_status), y = worr_health_dummy, fill = factor(rel_status))) +
  geom_bar(stat = "summary", fun = "mean", alpha = 0.7) +
  geom_text(stat = "summary", fun = "mean", aes(label = round(after_stat(y), 2)), vjust = -0.5) +
  scale_fill_brewer(palette = "Blues", guide = "none") +
  labs(
...
ggplot(main_germborn, aes(x = factor(germborn), y = health, fill = factor(germborn))) +
  ...
  labs(
    title = "Distribution of Health by German-Born Status",
    x = "German-Born Status",
    y = "Health"
  ) +
  theme_minimal(base_size = 14) +
...
ggplot(main_germborn, aes(x = factor(germborn), y = health_decline_2yrs, fill = factor(germborn))) +
  ...
  scale_x_discrete(labels = c("0" = "Born outside of Germany", "1" = "Born in Germany")) +
  labs(
    title = "Health Decline by German-Born Status",
    x = "German-Born Status",
    y = "Health Decline"
  ) +
  theme_minimal(base_size = 14) +
...
ggplot(main_germborn, aes(x = worr_health_dummy, fill = factor(germborn))) +
  ...
  labs(
    title = "Density of Health Worries by German-Born Status",
    x = "Health Worries",
    y = "Density"
  ) +
  theme_minimal(base_size = 14) +
...
ggplot(main_germborn, aes(x = factor(germborn), y = health_satisfaction, fill = factor(germborn))) +
  ...
  scale_x_discrete(labels = c("0" = "Born Outside of Germany", "1" = "Born in Germany")) +
  labs(
    title = "Health Satisfaction by German-Born Status",
    x = "German-Born Status",
    y = "Health Satisfaction"
  ) +
  theme_minimal(base_size = 14) +
...
ggplot(main, aes(x = life_satisfaction, y = health_satisfaction)) +
  ...
  labs(
    title = "Health Satisfaction vs. Life Satisfaction (Heatmap)",
    x = "Life Satisfaction",
    y = "Health Satisfaction"
  ) +
  theme_minimal(base_size = 14) +
...
ggplot(non_zero_coefficients, aes(x = reorder(Variable, Coefficient), y = Coefficient, fill = Coefficient > 0)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 12), # Adjust x-axis text size
    axis.text.y = element_text(size = 12, face = "bold"), # Make y-axis labels bold
    plot.title = element_text(face = "bold", size = 12, hjust = 0.5) # Center and style the title
  )
