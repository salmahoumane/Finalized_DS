...
my_packages <- c("tidyverse", "haven", "openxlsx", "labelled", "dplyr", "lubridate","ggplot2","skimr","ggcorrplot","patchwork","ggridges", "glmnet", "plm", "magrittr", "caret","waffle","randomForest","rpart","ranger","vip","pdp","rpart.plot","fastDummies")
...
ggplot(dummy_data, aes(x = as.factor(Value), y = Count, fill = as.factor(Value))) +
  ...
  facet_wrap(~ DummyVariable, scales = "free_y") +
  labs(
...
```{r}
# Calculate the Partial Dependence for married
pdp_married <- partial(
  object = rf_final, 
  pred.var = "married", 
  train = trainData,
...
```{r}
# Plot the PDP for  married
p1 <- autoplot(pdp_married) +
...
# Plot the PDP for white color job
p4 <- autoplot(pdp_white_collar) +
  ylab("Health Decline Prob")+
  xlab("White Color Job (Dummy)")+
  theme_minimal()
